...
## 7) Smoke test GraphQL in cloud

```sh
curl -sS -X POST \
  "https://$(az containerapp show -g rg-msk-dev -n ca-msk-graphql-dev --query properties.configuration.ingress.fqdn -o tsv)/graphql" \
  -H "Content-Type: application/json" \
  --data '{"query":"query { health serviceInfo { cosmosConfigured appInsightsConfigured cosmosDatabase cosmosContainer } }"}'
```
...
```sh
az containerapp show -g rg-msk-dev -n ca-msk-graphql-dev --query "properties.template.containers[0].image" -o tsv
az containerapp revision list -g rg-msk-dev -n ca-msk-graphql-dev -o table
az containerapp logs show -g rg-msk-dev -n ca-msk-graphql-dev --tail 100
```
...
- If GraphQL returns `Cannot POST /graphql`:
  - Check active image is not `mcr.microsoft.com/azuredocs/aci-helloworld:latest`.
  - Re-run `scripts/release-graphql.sh dev`.
- If first GraphQL call is slow:
